...
positions = sorted(range(len(total_score)), key=lambda x: total_score[x], reverse=True)
positions = [positions.index(i) + 1 for i in range(len(total_score))]

print("\nResults:")
print(f"{'Student':<10}{'Subjects':<20}{'Total Score':<15}{'Average Score':<15}{'Position':<10}")

for i in range(numof_student):
    student_subjects = ' '.join(map(str, reg_student[i]))
    print(f"{'Student ' + str(i + 1):<10}{student_subjects:<20}{total_score[i]:<15}{average_score[i]:<15.2f}{positions[i]:<10}")
